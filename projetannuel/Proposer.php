...
<html>
    ...
    <?php
        ...
        if (isset ($_POST['Proposer']))
        {
            ...
            $idvoiture=$bdd->prepare("SELECT idVehicule FROM Vehicule WHERE idUtilisateur = :id");
            $idvoiture=$bdd->execute(array(':id'=>$_SESSION['idUtilisateur']));

            $proposer=$bdd->prepare("INSERT INTO trajet(idUtilisateur, idVehicule, Pointdedepartlatitude, Pointdedepartlongitude, Pointdarriveelatitude, Pointdarriveelongitude, HeureDepart, HeureArrivee, NbrBagage, NbrBagageMax, PrixPersonne, DateDepart, DateArrivee) VALUES (:id, :idvoiture, :nom, :prenom, :age, :sexe, :tel, :adresse, :mail, :mdp)");
            $inscription -> execute(array(':id' => $_SESSION['idUtilisateur'], ':idvoiture' => $_POST['Prenom'],':age' => $_POST['Datedenaissance'],':sexe' => $_POST['Sexe'],':tel' => $_POST['Tel'],':adresse' => $_POST['Adresse'],':mail' => $_POST['Email'],':mdp' => $_POST['Mdp']));
